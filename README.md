...
- **자동 연결** — 마지막에 쓴 장비를 기억해 두 번째 실행부터는 선택 창 없이 바로 연결 (장비가 바뀌면 자동으로 선택 창으로 복귀)
...
#### 자동 연결 (두 번째 실행부터 선택 창 건너뛰기)

선택 창 하단의 **`Remember this device and connect automatically`** 를 체크한 채로 연결하면, 다음 실행부터는 **선택 창 없이 바로 그 장비에 연결**됩니다. 고정 자리에 장비를 두고 쓰는 경우에 편합니다.

| 상황 | 동작 |
|---|---|
| 저장된 장비가 그대로 있음 | 선택 창 없이 즉시 연결 |
| 같은 장비인데 USB 포트를 바꿔 꽂음 | **시리얼 번호로 찾아 자동 연결** (`USB0→USB1` 등 변경분은 자동 갱신) |
| 장비가 교체됨 / 안 보임 | 이유를 알려주는 안내와 함께 선택 창 표시 |
| 부팅 직후라 USB 인식이 덜 됨 | 최대 3회(약 2초) 재검색 후 판단 |

- **선택 창을 다시 띄우려면**: `Shift`를 누른 채 실행하거나, `RigolWidget.exe --select` 로 실행
- **실행 중 장비 교체**: 타이틀바 **우클릭 → `Change Device…`** (재시작 불필요, 새 장비의 모델·채널 수를 다시 인식)
- **바로가기로 특정 장비 지정**: `RigolWidget.exe --device "USB0::0x1AB1::0x0E11::DP8C200300243::INSTR"`
- 설정은 `%APPDATA%\RigolWidget\settings.json` 의 `AutoConnect` / `LastResource` 에 저장됩니다.
...
| **장비 변경** | 다른 장비로 전환 | 타이틀바 **우클릭 → `Change Device…`** |
...
| 도구 | 설명 | 힌트 | 제어 허용 필요 |
|---|---|:---:|:---:|
| `get_status` | 전 채널 측정값·설정값·출력·CV/CC·OCP/OVP·트립 상태 조회 | 읽기 전용 | — |
| `get_identity` | 모델명·IDN·채널 수 조회 | 읽기 전용 | — |
| `show_panel` | **대화창 안에 라이브 제어 패널 띄우기** (MCP Apps) | 읽기 전용 | — |
| `set_voltage` / `set_current` | 채널 전압 / 전류 리미트 설정 | 변경 | ✅ |
| `set_output` | 채널 출력 ON/OFF | 변경 | ✅ |
| `set_ocp` / `set_ovp` | 과전류 / 과전압 보호 활성·임계값 설정 | 변경 | ✅ |
| `clear_trip` | 보호 트립(OCP/OVP) 해제 | 변경 | ✅ |

각 도구는 MCP **툴 애노테이션**(`readOnlyHint` / `destructiveHint` / `idempotentHint` / `openWorldHint`)을 함께 노출합니다. AI 클라이언트는 이 힌트로 "조회 도구"와 "장비를 바꾸는 도구"를 구분할 수 있어, 읽기 도구는 자동 승인하고 변경 도구만 확인받는 식으로 동작할 수 있습니다. 조회 도구는 **구조화 출력(structuredContent + outputSchema)** 도 함께 반환하므로, 클라이언트가 텍스트를 파싱하지 않고 스키마대로 값을 읽습니다.

### 대화창 안의 제어 패널 (MCP Apps)

`show_panel` 도구를 호출하면 AI 대화창 안에 **실제 동작하는 제어 패널**이 렌더링됩니다. 전 채널 측정값이 보이고, 출력 ON/OFF·전압/전류 설정·트립 해제를 패널에서 바로 누를 수 있습니다.

![대화창 안의 MCP 제어 패널](docs/mcp-panel.png)

> *"리골 패널 띄워줘"* 라고 말하면 됩니다. 패널의 버튼도 **"MCP 제어 허용"** 이 켜져 있을 때만 활성화되며, 꺼져 있으면 위 그림처럼 안내가 표시됩니다.
>
> MCP Apps는 MCP 표준의 확장 기능이라 지원하는 클라이언트(Claude 데스크톱/웹, VS Code Copilot 등)에서만 패널이 보입니다. 지원하지 않는 클라이언트에서는 `show_panel`이 `get_status`와 같은 텍스트·구조화 결과만 돌려줍니다.
...
- **거부는 오류로 반환**: 제어가 꺼진 상태의 쓰기 시도는 정상 응답이 아니라 **도구 오류**로 돌아가므로, AI가 성공한 것으로 착각하지 않습니다.

### 프로토콜 / 호환성

내장 서버는 MCP C# SDK **2.2.0** 기반이며 최신 스펙 리비전 **2026-07-28**(스테이트리스 요청, `server/discover`, 캐싱 힌트)을 지원합니다. 동시에 `initialize` 핸드셰이크를 쓰는 **2025-11-25 이하 구형 클라이언트**도 그대로 접속됩니다(하이브리드 세션 모드). 쓰던 Claude Code/Desktop 설정을 바꿀 필요는 없습니다.
